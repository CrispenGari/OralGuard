...
import { classifyOralCancer } from "@/src/utils/react-query";
...
const Page = () => {
  ...
  const { isPending: predicting, mutateAsync } = useMutation({
    mutationKey: ["classify-oral-cancer"],
    mutationFn: classifyOralCancer,
  });
  ...
  const makeRecommendations = async () => {
    ...
    if (!!!state.uri) {
      setState((s) => ({
        ...s,
        error: "Please select a Oral Lesion image.",
      }));
      return;
    }
    ...
    const lesion = generateRNFile({
      name: state.fileName || "lesion.jpeg",
      uri: state.uri,
    });
    await mutateAsync({
      image: lesion,
      ...
      >
          ...
          <View style={{ flexDirection: "row", flex: 1 }}>
            ...
            >
              <Text style={{ fontFamily: FONTS.bold, fontSize: 25 }}>
                Oral Lesion Selection
              </Text>
              <Text style={{ fontFamily: FONTS.regular, color: COLORS.gray }}>
                Select a oral lesion or take it live from camera.
              </Text>
            </View>
          </View>
        ...
        >
          <View style={{ flexDirection: "row", flex: 1 }}>
            ...
            >
              <Text style={{ fontFamily: FONTS.bold, fontSize: 25 }}>
                Then, what kind of oral cancer do I have?
              </Text>
              <Text style={{ fontFamily: FONTS.regular, color: COLORS.gray }}>
          ...
          <Button
            ...
            title="Analyze Oral Lesion Image"
            style={{
              width: "100%",
